...
// Query: *[  _type == "lesson"  && _id == $id][0] {  _id,  title,  slug,  description,  video {    asset-> {      playbackId,      status,      data {        duration      }    }  },  content[]{    ...,    _type == "file" => {      "asset": asset->{        url,        originalFilename      }    }  },  "resources": resources[]{    "fileName": asset->originalFilename,    "fileUrl": asset->url,    "fileSize": asset->size,    description  },  completedBy,  "courses": *[_type == "course" && ^._id in modules[]->lessons[]->_id] | order(    select(tier == "free" => 0, tier == "pro" => 1, tier == "ultra" => 2)  ) {    _id,    title,    slug,    tier,    modules[]-> {      _id,      title,      lessons[]-> {        _id,        title,        slug,        completedBy      }    }  }}
